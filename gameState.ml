...
class state (mapW : int)
            (mapH : int) =
  object (this)
    val gameArray = generateMap mapW mapH
    ...
    method explode (xPos : int) (yPos : int) (b : bomb) =
      ...
      let directionalExplosion x y check =
        ...
        | Box _ ->
          ...
          if rand = 1 then
            gameArray.(x).(y) <-
              Powerup (new extrabomb (new point (x*xWIDTH) (y*yWIDTH))
                                      xWIDTH
                                      yWIDTH)
          else if rand = 2 then
            gameArray.(x).(y) <-
              Powerup (new firepower (new point (x*xWIDTH) (y*yWIDTH))
                                      xWIDTH
                                      yWIDTH)
          else explode' x y;
          false
    ...
    method makeEnemies (num : int) =
      enemylist <- List.init num
                             (fun _ -> new enemy (new point ((mapW - 2) * xWIDTH)
                                                            ((mapH - 2) * yWIDTH))
                                                 (yWIDTH / 2)
                                                 xWIDTH
                                                 yWIDTH)
    ...
    method moveEnemies =
      let moveEnemy = fun i enemy ->
        ...
        | Powerup _, _
        | Box _, _
        | Bomb _, _ -> enemy#move (newx * xWIDTH)
                                (newy * yWIDTH)
        | Exploding _, _
        | _, Exploding _ ->
          enemy#move (newx * xWIDTH)
                      (newy * yWIDTH);
          enemy#draw;
          enemy#draw;
      ...
      if moveEnemies = eNEMYDELAY then
        (List.iteri moveEnemy enemylist;
        moveEnemies <- 0)
      else
